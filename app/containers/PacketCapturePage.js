...
export default class PacketCapturePage extends React.Component {
		...
		return (
			<div>
				...
				<div className={stylesheet.packetDetailsContiner}>
					{(this.state.currentPacket != null) && <PacketContents packet={this.state.currentPacket}/>}
				</div>
			</div>
		);
	}
	...
	filterPackets(packetList) {
		if (this.state.filterEngine == null)
			return packetList.slice(packetList.length - this.state.displayCount, packetList.length);
	...
	handleFilterTextChanged(filterText) {
		this.setState(Object.assign(this.state, {
			...
			filterEngine: new PacketFilter(filterText)
		}));
	}
